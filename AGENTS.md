...
- `PROJECT_VERSION` identifies the current source state, not the latest published portable build. It may advance through several merged PRs while GitHub Releases remain on an older version; those intermediate code versions do not require tags or portable releases. Every user-facing PR handoff still includes a short, copy-ready release-note fragment written for users.
- Portable releases are accumulated deliberately instead of being published for every successful PR. When the maintainer chooses to release (or an urgent stability/security fix warrants one), use the current `PROJECT_VERSION` and consolidate all user-facing fragments since the previous published tag into notes headed by what changed, any update or migration steps, and relevant known limitations. GitHub-generated notes are the technical changelog supplement, not a substitute for this summary.
- Push a release tag only from the final merged `main` commit, and require it to exactly match the current `PROJECT_VERSION`. The tag triggers the verified Windows build, an informational VirusTotal scan, and a draft GitHub Release; inspect its security block, generated notes, ZIP, and checksum before publishing it manually. VirusTotal detections and integration failures warn but never block the draft; publishing remains a maintainer decision.
