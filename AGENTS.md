...
- `/backup` (owner-only) zips the whole `DATA_DIR` for export and restores a whitelist (`subscribers.json`, `stats_current.json`, `quarters/*.json`) on import; the owner also receives automatic backups (tag `#backup`) on subscribe/unsubscribe, quarter rotation, weekly (via the `last_backup_at` marker), and graceful shutdown. Replaces the former `/export` and `/import`.
