...
- Push a release tag only from the final merged `main` commit after its version change is present. The tag triggers the verified Windows build, an informational VirusTotal scan, and a draft GitHub Release; inspect its security block, generated notes, ZIP, and checksum before publishing it manually. VirusTotal detections and integration failures warn but never block the draft; publishing remains a maintainer decision.
