...
async def sync_stats_all(
    ...
    """
    Синхронизирует статистику пользователя по экспортам аниме и манги и избранному.
    
    При недоступности обоих экспортов возвращает исходные данные без изменений. При
    частичной недоступности продолжает обработку доступного экспорта. Обновляет
    тайтлы, агрегаты и избранное, сохраняя данные только при наличии изменений.
    
    Parameters:
    	fav: Предварительно полученные данные избранного; если не переданы,
    	избранное загружается самостоятельно.
    
    Returns:
    	tuple[dict, bool]: Актуализированные данные статистики и признак успешной
    	обработки хотя бы одного экспорта.
    
    Raises:
    	ProfilePrivacyError: Если профиль недоступен из-за настроек приватности.
    """
