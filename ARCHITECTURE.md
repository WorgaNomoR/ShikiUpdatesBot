...
- `.github/workflows/dependency-submission.yml` / `.github/scripts/validate_dependency_submission.py` — build-time-only submission of the resolved Python 3.12 dependency graph. GitHub's managed plain-pip graph job remains enabled, but submits unpinned root requirements without resolved versions or transitive edges. The explicit workflow scans the runtime-supported `Python` detector category from the repository root, so `requirements.txt`, `requirements-dev.txt`, and `requirements-build.txt` keep distinct repository-relative identities without an adapter or lockfile policy. The pinned action wrapper downloads the current Component Detection release at runtime; the validator therefore rejects a green action step unless all three manifests have direct dependencies and transitive edges in `output.json`.
