...
- `.github/workflows/dependency-submission.yml` / `.github/scripts/submit_dependency_snapshot.py` / `.github/scripts/validate_dependency_submission.py` — build-time-only submission of the resolved Python 3.12 dependency graph. GitHub's managed plain-pip graph job remains enabled, but submits unpinned root requirements without resolved versions or transitive edges. Component Detection's stable `PipReport` detector only discovers files named exactly `requirements.txt`, so it cannot preserve the identities of the canonical root `requirements-dev.txt` and `requirements-build.txt`. The explicit workflow instead runs `pip --dry-run --report` separately for all three files, constructs one Dependency Submission API snapshot with exact repository-relative paths, validates versioned direct dependencies and transitive edges, and only then submits it. The snapshot deliberately retains the legacy Component Detection detector identity and correlator so GitHub replaces the earlier manual source during manifest deduplication instead of treating the corrected graph as a competing detector. This keeps the manifests distinct without an adapter or lockfile policy.
