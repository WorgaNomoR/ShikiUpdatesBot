...
- `.github/workflows/dependency-submission.yml` — build-time-only submission of the resolved Python 3.12 dependency graph. GitHub's managed plain-pip graph job remains enabled, but submits unpinned root requirements without resolved versions or transitive edges. The explicit workflow scans only Pip manifests from the repository root, so `requirements.txt`, `requirements-dev.txt`, and `requirements-build.txt` keep distinct repository-relative identities without an adapter or lockfile policy.
